...
func (tx *Tx) init() {
	tx.Answers = NewAnswersClient(tx.config)
	tx.Attempt = NewAttemptClient(tx.config)
	tx.Attempt_answers = NewAttemptAnswersClient(tx.config)
...
// applies a query, for example: Answers.QueryXXX(), the query will be executed
